"""
Hperparameter tuning with Optuna + MLFlow.

- Optimizes Gradient Boosting Regression params set RMSE.
...
DEFAULT_OUT = Path("models/GradientBoostingRegressor_best_model.pkl")
...
def tune_model(
        ...
        experiment_name: str = "GradientBoostingRegressor",
        random_state: int = 42,
) -> tuple[Dict, Dict]:
    ...
    best_metrics = {"rmse": float(np.sqrt(mean_squared_error(y_eval, y_pred))),
        "mae": float(mean_absolute_error(y_eval, y_pred)),
        "r2": float(r2_score(y_eval, y_pred))
    }
